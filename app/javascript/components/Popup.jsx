...
class Popup extends React.Component {
    ...
    return (
      <div key={tag.id} className="card px-3 tag_canvas">
        <div className="row_">
          ...
          <div
            className="tagBox"
            style={{ textDecorationColor: `#${tag.hex}` }}
          >
            {tag.name}
          </div>
          <button
  ...
  deleteTags(event) {
    ...
    const cb = (response) => {
      ...
      this.props.sort_by_tag
        ? parseInt(event.target.value) === this.props.sort_by_tag.id
          ? this.props.nullify_sort_by()
          : ""
        : "";
      this.setState({
        tagList: newTags.filter((e) => e.id !== parseInt(event.target.value)),
      });
    };
  ...
  tagToggle(todo, tag) {
    return (event) =>
      event.target.checked
        ? this.createTagforTodo(todo, tag)
        : this.deleteTagforTodo(todo, tag);
  }
      ...
      <div key={1} className="card px-3 tag_canvas">
        <h6>
          <form onSubmit={this.unfocusForm}>
            <input type="checkbox" className="tag_check checkbox" />
            <input
              type="text"
              autoFocus
  ...
  unfocusForm(event) {
    ...
    const tagName =
      event.type === "submit" ? event.target["1"].value : event.target.value;
    this.addTag(tagName);
  }
  ...
  addTag(tagName) {
    ...
    const cb = (response) => {
      this.setState((state) => {
        ...
        return {
          tagList: state.tagList.concat(newTag),
    ...
    return (
      <div className="popup_canvas">
        ...
        <textarea
          ...
          className="edit_todo_form form-control col-12"
        />
        <br></br>
        {TagHTML}
        {this.state.newTag ? this.newTagHTML() : ""}
        <br></br>
        {!this.state.newTag && this.state.tagList.length < 10 ? (
          <input
            type="button"
            value="+"
            onClick={this.openNewTag}
            className="btn addTagBtn"
          />
        ) : (
          ""
        )}
